  ...
  <div>
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">实时同步监控 - {{ currentTask.taskName }}</h2>
        <StatusIndicator :status="taskStats.status" />
      </div>

      <div class="metrics-grid">
        <MetricCard title="总记录数" :value="taskStats.totalRecords" />
        <MetricCard title="已处理" :value="taskStats.processedRecords" />
        <MetricCard title="已扫描" :value="taskStats.scannedRecords" />
        <MetricCard title="失败分片" :value="taskStats.failedShards" />
      </div>
      ...
      <div class="logs-section">
        <h3 style="margin-bottom: 15px;">任务日志</h3>
    ...
    <div class="card">
      ...
      <div class="task-controls">
        <button
            ...
            :disabled="taskStats.status === 'RUNNING'"
        >
...
import { getTaskStats, startTask, stopTask } from '../services/api.js'
...
export default {
  ...
  setup() {
    ...
    const targetIcon = computed(() => `fas ${targetConfig.value.icon}`)
    const targetName = computed(() => targetConfig.value.name)
    const targetDescription = computed(() => targetConfig.value.description)

    const progressPercent = computed(() => {
      if (!taskStats.value.totalRecords) return 0
      return ((taskStats.value.processedRecords / taskStats.value.totalRecords) * 100).toFixed(1)
    })
    ...
    const startLocalTask = async () => {
      loading.value = true
      try {
        await startTask(store.state.config)
        store.commit('setCurrentTaskStats', { ...taskStats.value, status: 'RUNNING' })
        store.commit('addLog', {
          time: new Date().toLocaleTimeString(),
          message: `任务 ${store.state.config.taskName} 启动成功`
        })
        startMonitorPolling()
      } catch (error) {
        store.commit('addLog', {
    ...
    const stopLocalTask = async () => {
      loading.value = true
      try {
        await stopTask(currentTask.value.taskId)
        store.commit('setCurrentTaskStats', { ...taskStats.value, status: 'STOPPED' })
        store.commit('addLog', {
          time: new Date().toLocaleTimeString(),
          message: `任务 ${currentTask.value.taskId} 已停止`
        })
        stopMonitorPolling()
      } catch (error) {
        store.commit('addLog', {
    ...
    const startMonitorPolling = () => {
      stopMonitorPolling()
      fetchMonitorData()
      monitorInterval.value = setInterval(fetchMonitorData, 2000)
    }
    ...
    const fetchMonitorData = async () => {
      try {
        ...
        store.commit('setCurrentTaskStats', response.data.data)
      } catch (error) {
        store.commit('addLog', {
          time: new Date().toLocaleTimeString(),
